...
\right)=\sum\limits_{i=1}^{\infty} e^{-E_i/k_BT},
\end{align}
...
Z = \int d\bs{r}d\bs{r}' \rho_D(\bs{r},\bs{r}';\beta).
\end{align}
...
\end{align}
where $\mathcal{P}\bs{r}_M$ contains the same $N$ coordinates as $\bs{r}_M$, but with the particles relabeled. This permutation can happen via any number of 1-, 2-, and upto $N$-particle exchanges between adjacent time slices in the path. Thus, the state space of bosonic path integral is much larger than that of Boltzmannic path integral. Efficient sampling of permuting paths is a significant technical challenge. Fortunately, no uncontrolled approximation has been introduces, and exact simulations are possible via the Monte Carlo method.
...
One can still make progress at zero temperature by using a trial wave function along with an uncontrolled approximation.
