...
(a) H$_2$ bond length
\end{minipage}
\caption{vdW-DF optimized molecular candidate structures. C2/c and Cmca-12 have two types of H$_2$ molecules, whereas Cmca-4 has one.}
...
These functions remain roughly quantitatively similar across all densities explored.
...
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{f117_copt-c2c-rs1.27-ca1.75}\\
(a) Lattice deviations as functions of proton wave function exponent $C_p$.
\end{minipage}
\begin{minipage}{0.49\textwidth}
\centering
...
\end{align}
We expect some deviation from eq.~(\ref{eq:hsolid-x2-cp}) due to electron-proton and proton-proton correlations. As shown in Fig.~\ref{fig:prot-cp-opt}(a), the VMC MSDs are accurately approximated by eq.~(\ref{eq:hsolid-x2-cp}), whereas the DMC MSDs differ from the VMC ones in general.
