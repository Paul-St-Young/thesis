...
Non-diagonal supercell matrices can be used to maximize the minimum image radius, a.k.a. radius of the real-space Wigner-Seitz cell, $R_{WS}$.
...
U = \frac{1}{2}\sum\limits_{\alpha, \beta} \sum\limits_{i=1}^{N_\alpha} \sum\limits_{j=1}^{N_\beta, (j,\beta)\neq(i,\alpha)} u_{\alpha\beta}(r, \bs{R}_I),
\end{align}
where $\alpha, \beta$ denote electron spin species and $\bs{r}_j+\Delta\bs{r}_j$ is the quasi-particle coordinate of particle $j$ as determined by the back flow function as follows
\begin{align}
\Delta \bs{r}_j = \sum\limits_{j\neq i} \eta(r_{ij}) (\bs{r}_i-\bs{r}_j).
\end{align}
The Jastrow $u(r)$ and back flow $\eta(r)$ functions are optimized in VMC to lower the variational energy.
The single-particle orbitals in the determinant are taken as the vdW-DF Kohn-Sham orbitals with the lowest eigenvalues.
...
For an electron-ion simulation, the Slater-Jastrow wavefunction has an additional product for distinguishable ions ions and an ion-ion Jastrow.

\begin{align}
\Psi_{ei}(\bs{R}, \bs{R}_I) = \det\left( \{ \phi_i(\bs{r}_j+\Delta\bs{r}_j, \bs{r}_J+\Delta\bs{R}_J)\} \right) e^{-U(\bs{R},\bs{R}_I)}. \\
\end{align}
...
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{h45_xyz-copt}\\
...
Since proton RMSD is a rather direct measure of the shape of the wave function and DMC pushes the VMC RMSD towards its ground-state value, the optimal $C_p$ is obtained when DMC and VMC RMSDs agree.
...
For this purpose, we chose the DMC data for the C2/c-24 molecular crystal structure obtained by Drummond \textit{et al.}~\cite{Drummond2015}, because they are freely accessible online [REF] and are well approximated by a quadratic polynomial in density.
...
\caption{Fitting of reference equation of state.}
...
\includegraphics[width=\linewidth]{h117_evp-static-fsc1_e-vs-rho}
(a) energy-density relation
\end{minipage}
\begin{minipage}{0.49\textwidth}
\centering
%\includegraphics[width=\linewidth]{101se_hp}
\includegraphics[width=\linewidth]{h117_evp-static-fsc1_h-vs-p}
...
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{h117_evp-dynamic-e-vs-rho}
(a) energy-density relation
\end{minipage}
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{h117_evp-dynamic-h-vs-p}
...
Interestingly, this brings the dynamic-lattice QMC pressures into better agreement with static-lattice vdW-DF pressures, as we saw in Fig.~\ref{fig:hsolid-p-vs-n} that the static-lattice QMC pressures are systematically lower by $\sim 5$ GPa.
...
(c) dynamic lattice DMC ts=2
\end{minipage}
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{h117ga_dynamic-rs125-s3-gr}\\
(d) dynamic lattice DMC ts=1
\end{minipage}
\caption{Electronic pair correlation functions.}
...
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{h117ga_dynamic-c2c-s2-pp}\\
(a) C2/c-24
\end{minipage}
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{h117ga_dynamic-cmca12-s2-pp}\\
(b) Cmca-12
\end{minipage}
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{h117ga_dynamic-cmca4-s2-pp}\\
(c) Cmca-4
\end{minipage}
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{h117ga_dynamic-i41amd-s2-pp}\\
(d) I4$_1$/amd
\end{minipage}
