...
One goal of studying the many-body wave function is to understand electron correlation~\cite{Ceperley1999-wf}. As P. Dirac pointed out, knowing the Dirac/Schr\"odinger equation and the hamiltonian of the system is not enough for an understanding, because it ``leads to equations much too complicated to be soluble". Even a simple hamiltonian that contains only pair interactions, e.g., the Coulomb hamiltonian, complex correlation and phases can be present.
...
Thus, direct studies of experimental observables, the many-body wave function, and perhaps properties the exact density functional are crucial.
In this chapter, I will describe, in depth, the most understood many-body wave function for electronic structure, the Slater-Jastrow wave function and discuss what we have learned from it about the behavior of electrons.

\section{Historic Overview}

In condensed matter, the development of many-body wave function took off in the study of homogeneous quantum liquids, e.g., liquid helium and the homogeneous electron gas, a.k.a. jellium. Most studies made use of the variational principle, which states that given a Hamiltonian $\ham$, any trial wave function will have an energy value no less than that of the true ground state $\braket{\Psi_T|\ham|\Psi_T}\ge\braket{\Psi_0|\ham|\Psi_0}$. This principle allowed the pioneers to make educated guesses, then check their quality using the expectation value of $\ham$. Of course, the energy value is only one number. More are needed to truly quantify the accuracy of a wave function, but I digress.
...
Using the Gaskell RPA wave function as trial function, in 1980, D. M. Ceperley and B. J. Alder~\cite{Ceperley1980} solved the homogeneous electron gas problem in three spatial dimensions using exact QMC simulations.
...
For gravitational interaction $v(r)=Gm_1m_2/r^2$, eq.~(\ref{eq:wf-cusp}) implies
\begin{align}
...
\end{align}
where $\braket{}_{\Psi_T^2}$ denotes average over drift-diffusion random walks guided by $\Psi_T$ eq.~(\ref{eq:dmc-dfdt}) and $E_L(\bs{R}_t;\Psi_T)$ is the local energy of the trial wave function at walker position $\bs{R}_t$ during the random walk. If $\Psi_T$ is a good approximation for the ground state $\Psi_0$, then the imaginary time propagation can be cut off at a small equilibrium ``time'' $\tau$ and eq.~(\ref{eq:wf-fk}) has a compelling physical interpretation eq.~(6.13) in Ref.~\cite{Martin2016}
\begin{align}
\ln\Psi_0(\bs{R}_0) \approx \ln\Psi_T(\bs{R}_0) - \tau [
\left\langle E_L\right\rangle_{\Psi_T^2}(\bs{R}_0) - E_0
],
\end{align}
which says the correction to the exponent of $\Psi_T$ at $\bs{R}_0$ is proportional to the average of its local energy over a small region around $\bs{R}_0$ having size $O(\sqrt{N d \lambda\tau})$ in 3N-dimensional space. This observation is quite useful for practitioners of the DMC method, because it implies:
\begin{enumerate}
\item \textit{longer projections are needed for larger system} because the space over which $E_L$ needs to be averaged increase with $N$.
\item \textit{longer projections are needed for heavier particles} because they have smaller quantumness $\lambda$, thus smaller diffusion constant.
\item \textit{long projection time is needed to long-range properties of} $\Psi_T$, because the effect of projection is local and expands somewhat slowly with projection time $\propto \sqrt{\tau}$.
\end{enumerate}
